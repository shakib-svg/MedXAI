...
"""
CheXNet (DenseNet121 NIH14) + Integrated Gradients (Captum)

Outputs:
  overlays/<img>__ig_LABEL.png         overlay
  overlays/<img>__ig_LABEL__heat.png   heat-only
  raw/<img>__ig_LABEL__raw.png         raw grayscale (optional)
  preds_with_chexnet_ig_sample.csv
  metrics.json

Notes:
- IG explains the logit of a target class (pre-sigmoid), which is standard for attribution.
- The baseline matters. Default uses "imagenet_mean" baseline in normalized space.
"""
...
# ImageNet normalization used by TF
...
def load_image_chexnet(img_path: Path):
    ...
    x = TF(img).unsqueeze(0)  # [1,3,224,224] normalized
...
# -----------------------
def make_baseline(x: torch.Tensor, baseline: str) -> torch.Tensor:
    """
    x is normalized input [1,3,224,224].
    Returns baseline with same shape in normalized space.
    """
    if baseline == "zero":
        return torch.zeros_like(x)
    if baseline == "black":
        # black in pixel space => 0 -> normalized = (0-mean)/std
        b = torch.tensor((-IMN_MEAN / IMN_STD), dtype=x.dtype, device=x.device)
        return b.view(1, 3, 1, 1).expand_as(x)
    if baseline == "imagenet_mean":
        # mean pixel => normalized = 0
        return torch.zeros_like(x)
    if baseline == "blur":
        # blur x in normalized space (approx baseline)
        x_np = x.detach().cpu().numpy()[0].transpose(1, 2, 0)  # HWC
        x_blur = cv2.GaussianBlur(x_np, (31, 31), 0)
        xb = torch.from_numpy(x_blur.transpose(2, 0, 1)).unsqueeze(0).to(x.device).type_as(x)
        return xb
    raise ValueError("baseline must be in {zero, black, imagenet_mean, blur}")


def ig_attribution_map(ig: IntegratedGradients,
                       x: torch.Tensor,
                       class_idx: int,
    ...
    """
    IG on logit for class_idx. Returns 2D map in [0,1].
    mode:
      - pos: positive attributions only
      - abs: abs attributions
      - raw: signed (not recommended)
    """
    x = x.requires_grad_(True)

    # IG returns attr with same shape as x: [1,3,H,W]
    attr = ig.attribute(
        inputs=x,
        baselines=baseline,
        target=class_idx,
    ...
    # combine channels: sum of abs/pos usually used
    m = a.sum(axis=0)  # signed
...
def ig_noise_tunnel_map(ig: IntegratedGradients,
                        x: torch.Tensor,
                        class_idx: int,
                        baseline: torch.Tensor,
                        n_steps: int,
    ...
    """
    SmoothGrad Integrated Gradients via Captum NoiseTunnel.
    """
    nt = NoiseTunnel(ig)
    x = x.requires_grad_(True)
    attr = nt.attribute(
    ...
    a = attr[0].detach().cpu().numpy().astype(np.float32)
    m = a.sum(axis=0)
    if mode == "pos":
        m = np.maximum(m, 0.0)
    elif mode == "abs":
...
def main():
    ...
    ap.add_argument("--ig_internal_bs", type=int, default=16)
    ap.add_argument("--ig_mode", choices=["pos", "abs", "raw"], default="abs")
    ap.add_argument("--ig_baseline", choices=["imagenet_mean", "black", "zero", "blur"], default="imagenet_mean")

    # SmoothGrad-IG (NoiseTunnel)
    ap.add_argument("--use_noise_tunnel", action="store_true")
    ap.add_argument("--nt_samples", type=int, default=8)
    ap.add_argument("--nt_sigma", type=float, default=0.10)

    # Viz
    ap.add_argument("--xai_alpha", type=float, default=0.60)
    ap.add_argument("--xai_amplify", type=float, default=2.0)
    ap.add_argument("--xai_smooth_sigma", type=float, default=1.0)
    ...
    print(f"[INFO] IG steps={args.ig_steps} baseline={args.ig_baseline} mode={args.ig_mode} noise_tunnel={args.use_noise_tunnel}")
    ...
    for i in range(len(df)):
        ...
        # forward probs for metrics (no grad)
        with torch.no_grad():
            logits = model(x)
        ...
        for j in xai_classes:
            ...
            baseline = make_baseline(x, args.ig_baseline)

            if args.use_noise_tunnel:
                m01 = ig_noise_tunnel_map(
                    ig=ig,
                    x=x.detach().clone(),
                    class_idx=j,
                    baseline=baseline,
    ...
    out_csv = out_dir / "preds_with_chexnet_ig_sample.csv"
...
if __name__ == "__main__":
    main()
"""
CUDA_VISIBLE_DEVICES=0 python3 chexnet_ig_500.py \
  ...
  --out_dir /home/shakib/Desktop/S5/projet/xai_outputs/chexnet_ig_random500_seed0 \
  --sample_n 500 --sample_mode random --seed 0 \
  --thr 0.5 \
  --xai_on_predicted --topk_xai 2 \
  --xai_min_prob 0.30 \
  --ig_steps 64 --ig_baseline imagenet_mean --ig_mode abs \
  --xai_smooth_sigma 1.2 \
