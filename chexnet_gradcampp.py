...
class GradCAMPlusPlus:
    ...
    def __call__(self, x, class_idx: int):
        """
        Grad-CAM++:
          alpha_ij^k = (dY/dA)^2 / (2*(dY/dA)^2 + A*(dY/dA)^3)
          w_k = sum_{i,j} alpha_ij^k * ReLU(dY/dA_ij^k)
...
def main():
    ...
    ap.add_argument("--method", choices=["gradcam", "gradcampp"], default="gradcampp",
                    help="Méthode CAM: gradcam (baseline) | gradcampp (Grad-CAM++)")
    ...
    for _, row in df.iterrows():
        ...
        for ci in cam_classes:
            ...
            out_img = out_dir / "overlays" / f"{Path(img_name).stem}__{args.explain}__{method_tag}_{NIH14[ci]}.png"
        ...
        print(f"[{len(rows_out)}/{len(df)}] {img_name} | GT={gt_primary} | Pred={pred_primary} | explain={args.explain} | method={args.method}")
    ...
    out_csv = out_dir / f"results_{args.method}.csv"
...
python3 chexnet_gradcampp.py \
  ...
  --out_dir /home/shakib/Desktop/S5/projet/xai_outputs/gradcampp_pred \
  --task single_label \
  --nf_threshold 0.10 \
  --explain pred \
  --steps 10
