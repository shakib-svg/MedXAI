...
"""
CheXNet (DenseNet121 NIH14) + Grad-CAM (sample 500)
- Robust checkpoint loader:
    * strips common prefixes: module., densenet121., model.
    * FIX: maps classifier.{0,1}.weight/bias -> classifier.weight/bias
    * hard-check that classifier is loaded (otherwise predictions random)
- Robust image indexing across NIH layouts
- Saves overlays / heat-only / optional raw
- Saves per-class probabilities into CSV for debugging
"""
...
def build_chexnet():
    ...
    m.classifier = nn.Linear(m.classifier.in_features, 14)  # classifier.weight/bias
    return m
...
def _remap_key(k: str) -> str:
    ...
    # FIX: checkpoints often store classifier as Sequential(Dropout, Linear)
    # => classifier.1.weight/bias (or classifier.0.*)
    m = re.match(r"^classifier\.(\d+)\.(weight|bias)$", k)
    if m:
        idx = int(m.group(1))
        # if it's index 1 or 0, map to classifier.weight/bias
        if idx in (0, 1, 2):
            k = f"classifier.{m.group(2)}"

    # some older patterns (rare) where layers have ".norm.(\d)." etc
...
def load_chexnet_checkpoint(model, ckpt_path: Path, device):
    ...
    # pull state_dict from common containers
    if isinstance(ckpt, dict):
        if "state_dict" in ckpt:
            ...
            # if it's a dict of tensors already
            sd = ckpt
    else:
        sd = ckpt
    ...
    # --- HARD CHECK: classifier must be loaded ---
    miss_set = set(missing)
    if ("classifier.weight" in miss_set) or ("classifier.bias" in miss_set):
        print("[LOAD] Missing keys:", missing)
        print("[LOAD] Unexpected keys:", unexpected)
        raise RuntimeError(
            "Classifier NOT loaded from checkpoint (classifier.weight/bias missing). "
            "=> predictions random. Fix key mapping or verify checkpoint."
        )

    print(f"[LOAD] Missing={len(missing)} Unexpected={len(unexpected)}")
    if len(missing) or len(unexpected):
        # still okay if the classifier is loaded; but print details for debug
        if len(missing) <= 12:
...
def gradcam_from_logits(model, hooks: ActivationsAndGradients, logits: torch.Tensor, class_idx: int):
    ...
    if A is None or dY is None:
        raise RuntimeError("No activations/gradients captured. Bad target_layer?")

    with torch.no_grad():
        w = dY.mean(dim=(2, 3), keepdim=True)         # [1,C,1,1]
        cam = (w * A).sum(dim=1, keepdim=False)[0]    # [h,w]
        cam = F.relu(cam)
        cam = cam - cam.min()
        if cam.max() > 0:
            cam = cam / cam.max()
...
# -----------------------
# Save overlay / heat / raw
# -----------------------
def save_heat_and_overlay(rgb_u8: np.ndarray, cam01: np.ndarray, out_overlay_path: Path,
                          alpha_heat: float = 0.40, amplify: float = 1.0,
...
def get_target_layer(model, name: str):
    """
    default good layer: features.denseblock4 (conv features)
    accepts exact match or suffix match in named_modules()
    """
...
def main():
    ...
    ap.add_argument("--thr", type=float, default=0.5)      # <= évite le cas "tout positif"
    ...
    ap.add_argument("--debug_first", type=int, default=5)  # print prob stats on first N
    ...
    for i in range(len(df)):
        ...
        for j in topk_idx:
            ...
            if args.save_raw_cam:
                ...
                save_raw_gray(cam01, raw_path)
                raw_files.append(f"{base}__raw.png")

        # store per-class probs (important for debug)
    ...
    out_csv = out_dir / "preds_with_chexnet_gradcam_sample.csv"
...
CUDA_VISIBLE_DEVICES=0 python3 chexnet_gradcam_500.py \
  ...
  --out_dir /home/shakib/Desktop/S5/projet/xai_outputs/chexnet_gradcam_fixed_random500_seed0 \
