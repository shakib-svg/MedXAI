...
"""
PYLON + "LRP epsilon-rule (approx)" — FINAL PRO (500 images)

Why:
- Captum LRP / LayerLRP fails on AdaptiveMaxPool2d (no rule), common in many nets.
- This script implements a robust LRP-like relevance at input using:
      R = x * d(logit_c)/dx
  with optional positive-only relevance and robust normalization.

Outputs:
  overlays/<img>__lrp_LABEL.png
  overlays/<img>__lrp_LABEL__heat.png
  raw/<img>__lrp_LABEL__raw.png (optional)
  preds_with_lrp_sample.csv
  metrics.json
"""
...
# -----------------------
# LRP-approx core: R = x * grad(logit)
# -----------------------
def lrp_approx_cam01(model, x: torch.Tensor, class_idx: int, mode: str = "pos"):
    """
    Returns cam01 in input resolution [H,W].
    mode:
      - "pos": keep positive relevance only
      - "abs": absolute relevance magnitude
    """
    x = x.clone().detach().requires_grad_(True)
    model.zero_grad(set_to_none=True)

    out = model(x)
    logits = out.pred if hasattr(out, "pred") else out[0]  # [1,14]
    score = logits[0, class_idx]
    score.backward(retain_graph=False)

    grads = x.grad  # [1,1,H,W]
    rel = (x * grads).detach()[0, 0]  # [H,W]

    rel_np = rel.cpu().numpy().astype(np.float32)
    if mode == "pos":
        rel_np = np.maximum(rel_np, 0.0)
    elif mode == "abs":
        rel_np = np.abs(rel_np)
    else:
        raise ValueError("mode must be 'pos' or 'abs'")

    rel_np = rel_np - rel_np.min()
    if rel_np.max() > 0:
        rel_np = rel_np / rel_np.max()
    return rel_np


def normalize_cam_percentile(cam01: np.ndarray, p_low=5.0, p_high=99.0, gamma=0.9):
...
def main():
    ...
    ap.add_argument("--lrp_mode", choices=["pos", "abs"], default="pos")
    ...
    print(f"[INFO] LRP-approx mode={args.lrp_mode} topk={args.topk}")
    print(f"[INFO] thr={args.thr} tau_nf={args.tau_nf}")
    ...
    for i in range(len(df)):
        ...
        topk = min(args.topk, 14)
        topk_idx = np.argsort(-prob)[:topk].tolist()

        overlay_files, heat_files, raw_files = [], [], []

        for j in topk_idx:
            cam01 = lrp_approx_cam01(model, x, class_idx=j, mode=args.lrp_mode)
            ...
            save_heat_and_overlay(rgb_u8, cam_viz, overlay_path,
    ...
    out_csv = out_dir / "preds_with_lrp_sample.csv"
...
python3 pylon_lrp_500.py \
  ...
  --out_dir /home/shakib/Desktop/S5/projet/xai_outputs/lrp_approx_final_random500_seed0 \
  --img_size 256 \
  --thr 0.1 \
  --tau_nf 0.07 \
